...
def push_data(id, issue_type, severity):
     ...
     firebase_push = pyrebase.initialize_app(config)
     db_push = firebase_push.database()
     if(issue_type == 0):
         iss = "electric"
     elif(issue_type == 1):
         iss = "crime"   
     ...
     db_push.child("UserInfo").child(id).update({"issue":iss})
     if(int(severity) == 0):
          ss = "Not Severe"
     else:
         ss = "Severe"     

     db_push.child("UserInfo").child(id).update({"severity":ss})
